...
class SCPRelativePositionBias(RelativePositionBiasBase):
    ...
    def create_grid_all_dists(self, x_size=11, y_size=11):
        ...
        for x in range(x_size):
            for y in range(y_size):
                ...
                a[np.isnan(a)]=0
                dists = np.sqrt((x_arr - cell[0])**2 + (y_arr - cell[1])**2).reshape(1,x_size,y_size) #* left_to_right
                if y==0:
                    dists_align_y = dists
            ...
            dists_align_y = dists_align_y.reshape(1,y_size,x_size,y_size)
